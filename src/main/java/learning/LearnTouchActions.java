...
public class LearnTouchActions {
	public static void main(String[] args) throws MalformedURLException, InterruptedException {
		...
		// Downgrade appium version to 7.X.Y for below methods (TouchActions and
		// MultiTouchAction) to work
		/*
		 * // Swipe up new TouchAction<>(driver).press(PointOption.point((int) (maxX *
		 * 0.5), (int) (maxY * 0.8)))
		 * .waitAction(WaitOptions.waitOptions(Duration.ofMillis(2000)))
		 * .moveTo(PointOption.point((int) (maxX * 0.5), (int) (maxY *
		 * 0.2))).release().perform(); // Swipe down new
		 * TouchAction<>(driver).press(PointOption.point((int) (maxX * 0.5), (int) (maxY
		 * * 0.2))) .waitAction(WaitOptions.waitOptions(Duration.ofMillis(2000)))
		 * .moveTo(PointOption.point((int) (maxX * 0.5), (int) (maxY *
		 * 0.8))).release().perform(); // Swipe left new
		 * TouchAction<>(driver).press(PointOption.point((int) (maxX * 0.8), (int) (maxY
		 * * 0.5))) .waitAction(WaitOptions.waitOptions(Duration.ofMillis(2000)))
		 * .moveTo(PointOption.point((int) (maxX * 0.2), (int) (maxY *
		 * 0.5))).release().perform(); // Swipe Right new
		 * TouchAction<>(driver).press(PointOption.point((int) (maxX * 0.2), (int) (maxY
		 * * 0.5))) .waitAction(WaitOptions.waitOptions(Duration.ofMillis(2000)))
		 * .moveTo(PointOption.point((int) (maxX * 0.8), (int) (maxY *
		 * 0.5))).release().perform(); //Multitouch new MultiTouchAction(driver)
		 * .add(new TouchAction<>(driver).press(PointOption.point((int) (maxX * 0.5),
		 * (int) (maxY * 0.8)))
		 * .waitAction(WaitOptions.waitOptions(Duration.ofMillis(2000)))
		...
		finger1.addAction(input.createPointerMove(Duration.ofMillis(0), Origin.viewport(), (int) (maxX * 0.25),
				(int) (maxY * 0.25)));
		...
		finger2.addAction(input2.createPointerMove(Duration.ofMillis(0), Origin.viewport(), (int) (maxX * 0.75),
				(int) (maxY * 0.75)));
		finger2.addAction(input2.createPointerDown(MouseButton.LEFT.asArg()));
		finger2.addAction(input2.createPointerMove(Duration.ofMillis(2000), Origin.viewport(), (int) (maxX * 0.5),
				(int) (maxY * 0.5)));
		...
		swipeUp1.addAction(input3.createPointerMove(Duration.ofMillis(0), Origin.viewport(), (int) (maxX * 0.5),
				(int) (maxY * 0.8)));
		swipeUp1.addAction(input3.createPointerDown(MouseButton.LEFT.asArg()));
		swipeUp1.addAction(input3.createPointerMove(Duration.ofMillis(2000), Origin.viewport(), (int) (maxX * 0.5),
				(int) (maxY * 0.2)));
